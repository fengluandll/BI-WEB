...
class DataSource extends PureComponent {
  ...
  columns = [{
    ...
    dataIndex: 'dsDisplay',
    key: 'dsDisplay',
    sorter: (a, b) => a.dsDisplay > b.dsDisplay,
    render: (text, record) => {
      const url = `/dlColumn/${record.id}`;
      return (
        <div className="ant-list-item-meta-content">
          <h4 className="ant-list-item-meta-title">
            <Link to={url}>{text}</Link>
          </h4>
          <div className="ant-list-item-meta-description">{record.dsName}</div>
        </div>
      );
    },
  }, {
    ...
    render: (text, record) => (
      <ButtonGroup>
        <Tooltip placement="top" title="新建仪表板">
