...
class ChartHelper {
   ...
   * @returns {*}
   */
  initDataSetBar = (data, config, chartType) => {
    const { styleConfigs, dataConfig } = config;
    const ds = new DataSet();
    const dv = ds.createView();
    dv.rows = ObjectUtils.quickDeepClone(data);
    this.initTransform(dv, styleConfigs, chartType);
    this.fill(dv, data, styleConfigs);
    const x = dataConfig.dimension[0] ? dataConfig.dimension[0].rscDisplay : '维度';
    const y = dataConfig.measure[0] ? dataConfig.measure[0].rscDisplay : '度量';
    const color = dataConfig.legend[0] ? dataConfig.legend[0].rscDisplay : '图例';
    // 按字段排序
    let order;
    let fields;
    if(styleConfigs.orderBy == 1 ){
      fields = ['x'];
      order = 'ASC'
    }else{
      fields = ['y'];
    ...
    dv.transform({
      type: 'sort-by',
      fields: fields, // 根据指定的字段集进行排序，与lodash的sortBy行为一致
      order: order,        // 默认为 ASC，DESC 则为逆序
    });
    // 字段重命名
    dv.transform({
      type: 'rename',
   ...
   * @returns {*}
   */
  initDataSet = (data, config, chartType) => {
    const { styleConfigs, dataConfig } = config;
    const ds = new DataSet();
    const dv = ds.createView();
    dv.rows = ObjectUtils.quickDeepClone(data);
